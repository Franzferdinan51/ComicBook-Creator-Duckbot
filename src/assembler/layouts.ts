...
export function layoutPage(
  ...
  if (explicit === 'strip-3' || (!explicit && n === 3)) {
    // 3 panels in a row. Prefer a wide cinematic cell so storyboard
    // style pages and 16:9 renders don't get collapsed into square
    // thumbnails. Portrait pages still get a contained strip because
    // the height is capped by the usable page area.
    const cols = 3;
    const usableW = pageWidth - 2 * margin;
    const w = (usableW - gap * (cols - 1)) / cols;
    const h = w * 9 / 16;
