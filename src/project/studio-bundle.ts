...
export interface StudioBundle {
  ...
  productionRunManifest: ComicResult['productionRunManifest'];
...
export function buildStudioBundle(jobId: string, result: ComicResult): StudioBundle {
  return {
    ...
    artifactPaths: {
      ...
      productionRunManifestPath: result.productionRunManifestPath,
    ...
    availability: {
      ...
      agentWorkflowPackage: Boolean(result.agentWorkflowPackagePath || result.agentWorkflowPackage),
      productionRunManifest: Boolean(result.productionRunManifestPath || result.productionRunManifest),
      screenplay: Boolean(result.screenplayPath || result.project),
      directorBrief: Boolean(result.directorBriefPath || result.project),
      ...
      musicCuePackage: Boolean(result.musicCuePackagePath || result.musicCuePackage),
      seriesPackage: Boolean(result.seriesPackagePath || result.seriesPackage),
      storyboardPackage: Boolean(result.storyboardPackagePath || (result.project && result.pages.length > 0)),
      trailerPackage: Boolean(result.trailerPackagePath || result.trailerPackage),
      videoPackage: Boolean(result.videoPackagePath || result.videoPackage),
      animaticTimeline: Boolean(result.animaticTimelinePath || (result.project && result.pages.length > 0)),
